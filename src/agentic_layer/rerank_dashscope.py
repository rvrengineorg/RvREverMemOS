"""
DashScope (Alibaba Cloud) Rerank Service Implementation

Reranking service using DashScope compatible API (qwen3-rerank).
...
@dataclass
class DashScopeRerankConfig:
    """DashScope rerank service configuration"""
...
class DashScopeRerankService(RerankServiceInterface):
    """DashScope reranking service implementation"""

    def __init__(self, config: Optional[DashScopeRerankConfig] = None):
        if config is None:
        ...
        logger.info(
            f"Initialized DashScopeRerankService | model={config.model} | url={config.base_url}"
        )
    ...
    async def _send_rerank_request_batch(
        ...
        """Send rerank request batch to DashScope compatible API"""
        await self._ensure_session()

        # DashScope qwen3-rerank compatible API format (flat structure)
        # No manual Qwen prompt template needed — API handles it internally
        request_data = {
        ...
        # Map instruction to DashScope's instruct parameter
        if instruction:
            request_data["instruct"] = instruction
        ...
        async with self._semaphore:
            for attempt in range(self.config.max_retries):
                try:
                    ...
                    logger.warning(
                        f"DashScope rerank timeout (attempt {attempt + 1}/{self.config.max_retries})"
                    )
                    if attempt < self.config.max_retries - 1:
                        await asyncio.sleep(2**attempt)
                        continue
                    ...
                    logger.warning(
                        f"DashScope rerank client error (attempt {attempt + 1}/{self.config.max_retries}): {e}"
                    )
                    if attempt < self.config.max_retries - 1:
                        await asyncio.sleep(2**attempt)
                        continue
                    ...
                    logger.error(f"Unexpected error in DashScope rerank request: {e}")
                    raise RerankError(f"Unexpected rerank error: {e}")

    def _parse_response(self, json_body: Dict[str, Any]) -> Dict[str, Any]:
        """Parse DashScope API response

        Compatible API (qwen3-rerank) response format (flat):
        {
            "id": "...",
            "model": "qwen3-rerank",
            "results": [
        ...
        Native API (gte-rerank-v2) response format (nested):
        {
            "output": {"results": [...]},
            "usage": {"total_tokens": 79},
            "request_id": "..."
        ...
        # Compatible API returns results at top level;
        # Native API wraps inside "output" key
        ...
        return {
            "scores": scores,
            "input_tokens": json_body.get("usage", {}).get("total_tokens", 0),
            "request_id": json_body.get("request_id") or json_body.get("id"),
        }
    ...
    async def rerank_memories(
        ...
        """
        Rerank memories using DashScope API
